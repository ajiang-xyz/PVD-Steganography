...
def rgbChannels(loadedImage,message='',quantizationWidths=[],traversalOrder=[],verbose=_C):
    if message=='':
        if verbose:print(_D)
    quantizationWidths=validateQuantization(quantizationWidths,verbose);traversalOrder=validateTraversal(traversalOrder,verbose);pixelPairs=pixelArrayToZigZag(loadedImage,3,2)
    if message=='':
            ...
            if len(pixelPair)==2 and currentMessageIndex<len(messageBinary)-1:
                ...
                for traversedPixelPair in traversedPixelPairs:
                    ...
                    if testingPair[0]<0 or testingPair[1]<0 or testingPair[0]>255 or testingPair[1]>255:
                        ...
                        elif currentMessageIndex==len(messageBinary):currentMessageIndex=len(messageBinary);postEncodingValues+=traversedPixelPair;continue
                ...
                pixelIndicesDict=dict(sorted(dict(zip([key[1]for key in pixelIndicesDict.keys()],postEncodingValues)).items()));reversedPaired=list(groupImagePixels([pixel for pixel in pixelIndicesDict.values()],3));newPixels+=reversedPaired
        ...
        newPixels=list(groupImagePixels(newPixels,loadedImage.size[0]));newPixels=pixelArrayToZigZag(newPixels,1,loadedImage.size[0],loadedImage.size[0],loadedImage.size[1]);array=np.array(newPixels,dtype=np.uint8);savedImage=PIL.Image.fromarray(array);savedImage.save('./IO/outColor.png');return returnValue
def singleChannel(loadedImage,message='',quantizationWidths=[],verbose=_C):
    if message=='':
        if verbose:print(_D)
    quantizationWidths=validateQuantization(quantizationWidths,verbose);pixelPairs=pixelArrayToZigZag(loadedImage,1,2)
    if message=='':
        messageBinary=''
        for pixelPair in pixelPairs:
            if len(pixelPair)==2:
        ...
        returnValue=_A
        if currentMessageIndex!=len(messageBinary):
            print(f"Warning: only encoded {len(messageBinary[0:currentMessageIndex])} of {len(messageBinary)} bits ({round(len(messageBinary[0:currentMessageIndex])/len(messageBinary),2)}%)");print(f"\nOriginal message: {message}");returnValue=_C
            if verbose==_A:
                width=os.get_terminal_size()[0];printableMessageLines=list(groupImagePixels(messageBinary,width));printableUnderlinings=list(groupImagePixels('~'*len(messageBinary[0:currentMessageIndex]),width));print(_F)
                for (printableMessageLine,printableUnderlining) in itertools.zip_longest(printableMessageLines,printableUnderlinings,fillvalue=''):
                    print(f"{printableMessageLine}")
...
@executionTime
def test():
    print('Beginning execution...');imagePath='./IO/inColor.png';loadedImage=PIL.Image.open(imagePath)
    if'RGB'in loadedImage.mode.upper():returnValue=rgbChannels(loadedImage,message='hello world! how are you doing?',verbose=_A)
